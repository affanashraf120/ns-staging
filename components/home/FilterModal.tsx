...
import nurse_types from '../../data/nurse-types.json'
...
const FilterModal = (props: JobFiltersProps) => {
  ...
  const useArrayState = (
    state: Array<string>
  ): [Array<string>, (s: string) => void, () => void] => {
    const [_innerState, _innerSetter] = useState(state);
    const setter = (value: string) => {
      if (_innerState.includes(value))
        _innerSetter(_innerState.filter((s) => s !== value));
  ...
  return (
    <div>
      ...
      {show ? (
        <div className="fixed inset-0 h-full bg-black bg-opacity-50 flex flex-col justify-center items-center z-10">
          <div className="w-full max-w-2xl">
            <div className="bg-gray-200 flex items-center justify-between py-4 px-5 border-b border-gray-300">
              ...
              {job_types.map(({ name, slug }) => (
                <div key={slug} className="flex justify-between items-center">
                  ...
                  <input
                    ...
                    name={slug}
                    className="h-5 w-5"
                    checked={jobTypes.includes(slug)}
                    onChange={() => setJobType(slug)}
                  />
                </div>
              ))}
              ...
              {nurse_types.map((nurseType) => (
                ...
                >
                  ...
                  <input
                    ...
                    name="nurse-type"
                    className="h-5 w-5"
                    checked={nurseTypes.includes(nurseType.slug)}
                    onChange={() => setNurseTypes(nurseType.slug)}
                  />
                </div>
              ))}
            </div>
